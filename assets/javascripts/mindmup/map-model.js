...
MAPJS.MapModel = function (layoutCalculatorArg, selectAllTitles, clipboardProvider, defaultReorderMargin) {
  ...
  var self = this,
      ...
      updateCurrentLayout = function (newLayout, sessionId) {
        ysy.log.debug("updateCurrentLayout", "redraw");
  ...
  this.followURL = function (id) {   //  HOSEK
    if (id === 'toolbar') {
      var subIdea = currentlySelectedIdea();
    } else {
      subIdea = self.findIdeaById(id);
    }
    if (subIdea) {
      self.dispatchEvent('followURL', subIdea);
    }
  };
  ...
  this.editNodeData = function (source) {
    if (!isEditingEnabled) {
      return false;
    }
    if (source) {
      analytic('editNodeData', source);
    }
    if (!isInputEnabled) {
      return false;
    }
    self.dispatchEvent('nodeEditDataRequested', currentlySelectedIdeaId);
  };
  ...
  this.toggleOneSide = function (source) {
    idea.oneSideOn = !idea.oneSideOn;
    //idea.toggleOneSide();
    idea.dispatchEvent("changed");
    self.dispatchEvent("saveSettings", idea);
  };
